...
namespace TYLDDB.Utils.FastCache.ConcurrentDictionary
{
    ...
    /// 使用并发词典来实现高并发的稳定性。
    /// </summary>
    public abstract class CdLongDictionary
    {
        /// <summary>
        ...
        /// <param name="value">Value to match<br />要匹配的值</param>
        /// <returns>List of keys<br />键的列表</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(long value) => await Task.FromResult(GetKeysByValue(value));
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the operation is successful.<br />操作是否成功。</returns>
        public virtual async Task<bool> SetAsync(string key, long value) => await Task.FromResult(Set(key, value));
        ...
        public virtual Dictionary<string, long> GetAllCache() => new Dictionary<string, long>(_cache);
    }
}
