...
namespace TYLDDB.Utils.FastCache.ConcurrentDictionary
{
    ...
    /// 使用并发词典来实现高并发的稳定性。
    /// </summary>
    public abstract class CdDecimalDictionary
    {
        /// <summary>
        /// Thread-safe dictionary to store cache data.<br />
        /// 线程安全的字典，用于存储缓存数据。
        /// </summary>
        private readonly ConcurrentDictionary<string, decimal> _cache = new ConcurrentDictionary<string, decimal>();
        ...
        /// <param name="key">Key<br />键</param>
        /// <returns>Value<br />值</returns>
        public virtual async Task<decimal?> GetByKeyAsync(string key) => await Task.FromResult(GetByKey(key));
        ...
        /// <param name="value">Value to match<br />要匹配的值</param>
        /// <returns>List of keys<br />键的列表</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(decimal value) => await Task.FromResult(GetKeysByValue(value));
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the operation is successful.<br />操作是否成功。</returns>
        public virtual async Task<bool> SetAsync(string key, decimal value) => await Task.FromResult(Set(key, value));
        ...
        public virtual Dictionary<string, decimal> GetAllCache() => new Dictionary<string, decimal>(_cache);
    }
}
