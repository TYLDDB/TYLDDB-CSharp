...
namespace TYLDDB.Utils.FastCache.ConcurrentDictionary
{
    ...
    public class CdDoubleDictionary
    {
        ...
        /// 线程安全的字典，用于存储缓存数据。
        /// </summary>
        private readonly ConcurrentDictionary<string, double> _cache = new ConcurrentDictionary<string, double>();
        ...
        /// <param name="key">Key<br />键</param>
        /// <returns>Value<br />值</returns>
        public virtual async Task<double?> GetByKeyAsync(string key) => await Task.FromResult(GetByKey(key));
        ...
        /// <param name="value">Value to match<br />要匹配的值</param>
        /// <returns>List of keys<br />键的列表</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(double value) => await Task.FromResult(GetKeysByValue(value));
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the operation is successful.<br />操作是否成功。</returns>
        public virtual async Task<bool> SetAsync(string key, double value) => await Task.FromResult(Set(key, value));
        ...
        public virtual Dictionary<string, double> GetAllCache() => new Dictionary<string, double>(_cache);
    }
}
