...
namespace TYLDDB.Utils.FastCache.SemaphoreThreadLock
{
    ...
    {
        private readonly Dictionary<string, bool> keyValueDict;   // 存储键->值映射
        private readonly Dictionary<bool, HashSet<string>> valueKeyDict; // 存储值->键的映射
        ...
        {
            keyValueDict = new Dictionary<string, bool>();
            valueKeyDict = new Dictionary<bool, HashSet<string>>();
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Key (List)<br />键 (List)</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(bool value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the Settings are successful.<br />是否设置成功</returns>
        public virtual async Task<bool> SetAsync(string key, bool value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        public virtual Dictionary<string, bool> GetAllCache()
        {
            lock (semaphore)
            {
                // 返回完整的键值对字典
                return new Dictionary<string, bool>(keyValueDict);
            }
        }
        ...
        /// </summary>
        /// <returns>Key-value pair<br />键值对</returns>
        public virtual async Task<Dictionary<string, bool>> GetAllCacheAsync()
        {
            await semaphore.WaitAsync();
            try
            {
