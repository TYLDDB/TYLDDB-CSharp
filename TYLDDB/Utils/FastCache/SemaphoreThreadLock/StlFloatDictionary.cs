...
namespace TYLDDB.Utils.FastCache.SemaphoreThreadLock
{
    ...
    /// </summary>
    public abstract class StlFloatDictionary
    {
        private readonly Dictionary<string, float> keyValueDict;   // 存储键->值映射
        private readonly Dictionary<float, HashSet<string>> valueKeyDict; // 存储值->键的映射
        ...
        {
            keyValueDict = new Dictionary<string, float>();
            valueKeyDict = new Dictionary<float, HashSet<string>>();
        ...
        /// <param name="key">Key<br />键</param>
        /// <returns>Value<br />值</returns>
        public virtual async Task<float?> GetByKeyAsync(string key)
        {
        ...
        public virtual List<string> GetKeysByValue(float value)
        {
            lock (semaphore)
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Key (List)<br />键 (List)</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(float value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the Settings are successful.<br />是否设置成功</returns>
        public virtual async Task<bool> SetAsync(string key, float value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        public virtual Dictionary<string, float> GetAllCache()
        {
            lock (semaphore)
            {
                // 返回完整的键值对字典
                return new Dictionary<string, float>(keyValueDict);
            }
        }
        ...
        /// </summary>
        /// <returns>Key-value pair<br />键值对</returns>
        public virtual async Task<Dictionary<string, float>> GetAllCacheAsync()
        {
            await semaphore.WaitAsync();
            try
            {
