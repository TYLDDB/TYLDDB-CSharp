...
namespace TYLDDB.Utils.FastCache.SemaphoreThreadLock
{
    ...
    /// </summary>
    public abstract class StlShortDictionary
    {
        private readonly Dictionary<string, short> keyValueDict;   // 存储键->值映射
        private readonly Dictionary<short, HashSet<string>> valueKeyDict; // 存储值->键的映射
        ...
        {
            keyValueDict = new Dictionary<string, short>();
            valueKeyDict = new Dictionary<short, HashSet<string>>();
        ...
        /// <param name="key">Key<br />键</param>
        /// <returns>Value<br />值</returns>
        public virtual async Task<short?> GetByKeyAsync(string key)
        {
        ...
        public virtual List<string> GetKeysByValue(short value)
        {
            lock (semaphore)
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Key (List)<br />键 (List)</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(short value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the Settings are successful.<br />是否设置成功</returns>
        public virtual async Task<bool> SetAsync(string key, short value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        public virtual Dictionary<string, short> GetAllCache()
        {
            lock (semaphore)
            {
                // 返回完整的键值对字典
                return new Dictionary<string, short>(keyValueDict);
            }
        }
        ...
        /// </summary>
        /// <returns>Key-value pair<br />键值对</returns>
        public virtual async Task<Dictionary<string, short>> GetAllCacheAsync()
        {
            await semaphore.WaitAsync();
            try
            {
