...
namespace TYLDDB.Utils.FastCache.SemaphoreThreadLock
{
    ...
    {
        private readonly Dictionary<string, int> keyValueDict;   // 存储键->值映射
        private readonly Dictionary<int, HashSet<string>> valueKeyDict; // 存储值->键的映射
        ...
        public StlIntegerDictionary()
        {
            ...
            valueKeyDict = new Dictionary<int, HashSet<string>>();
        ...
        /// <param name="key">Key<br />键</param>
        /// <returns>Value<br />值</returns>
        public virtual async Task<int?> GetByKeyAsync(string key)
        {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Key (List)<br />键 (List)</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(int value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the Settings are successful.<br />是否设置成功</returns>
        public virtual async Task<bool> SetAsync(string key, int value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        public virtual Dictionary<string, int> GetAllCache()
        {
            lock (semaphore)
            {
                // 返回完整的键值对字典
                return new Dictionary<string, int>(keyValueDict);
            }
        }
        ...
        /// </summary>
        /// <returns>Key-value pair<br />键值对</returns>
        public virtual async Task<Dictionary<string, int>> GetAllCacheAsync()
        {
            await semaphore.WaitAsync();
            try
            {
