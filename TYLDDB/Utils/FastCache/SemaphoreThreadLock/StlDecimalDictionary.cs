...
namespace TYLDDB.Utils.FastCache.SemaphoreThreadLock
{
    ...
    /// </summary>
    public abstract class StlDecimalDictionary
    {
        private readonly Dictionary<string, decimal> keyValueDict;   // 存储键->值映射
        private readonly Dictionary<decimal, HashSet<string>> valueKeyDict; // 存储值->键的映射
        ...
        {
            keyValueDict = new Dictionary<string, decimal>();
            valueKeyDict = new Dictionary<decimal, HashSet<string>>();
        ...
        /// <param name="key">Key<br />键</param>
        /// <returns>Value<br />值</returns>
        public virtual async Task<decimal?> GetByKeyAsync(string key)
        {
        ...
        public virtual List<string> GetKeysByValue(decimal value)
        {
            lock (semaphore)
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Key (List)<br />键 (List)</returns>
        public virtual async Task<List<string>> GetKeysByValueAsync(decimal value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        /// <param name="value">Value<br />值</param>
        /// <returns>Whether the Settings are successful.<br />是否设置成功</returns>
        public virtual async Task<bool> SetAsync(string key, decimal value)
        {
            await semaphore.WaitAsync();
            try
            {
        ...
        public virtual Dictionary<string, decimal> GetAllCache()
        {
            lock (semaphore)
            {
                // 返回完整的键值对字典
                return new Dictionary<string, decimal>(keyValueDict);
            }
        }
        ...
        /// </summary>
        /// <returns>Key-value pair<br />键值对</returns>
        public virtual async Task<Dictionary<string, decimal>> GetAllCacheAsync()
        {
            await semaphore.WaitAsync();
            try
            {
